...
export default function ItemFormModal({ item, onClose, onSave, title }) {
  ...
  const [cachedImageUrl, setCachedImageUrl] = useState(null) // image depuis pokemon_products ou upload
  const [cachedApiProductId, setCachedApiProductId] = useState(null)
  const [imageUploading, setImageUploading] = useState(false)
  const [imagePreview, setImagePreview]     = useState(null) // object URL local pour aperçu immédiat
  ...
  useEffect(() => {
    if (item) {
      ...
      setImagePreview(null) // pas de preview local en mode edit
    }
  }, [item])

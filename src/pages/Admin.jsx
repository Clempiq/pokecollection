...
function SetsManager() {
  ...
  // ── Import automatique depuis TCGdex ──────────────────────────────────────
  const importFromTCGdex = async () => {
    ...
    try {
      ...
      // Déterminer l'ordre des séries (par ordre d'apparition dans l'API)
      ...
      // Grouper par série et attribuer sort_order dans chaque série
      const seriesSortMap = {}
      ...
      // Upsert via tcgdex_id si la colonne existe, sinon insert par nom
      // On fait un upsert sur (series, name) pour éviter les doublons
      let inserted = 0
      let updated = 0
      ...
      // Update series_order for existing sets (if their series is now known)
